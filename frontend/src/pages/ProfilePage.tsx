  ...
  return (
    <div className="pt-16 min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {!editing && (
              <button
                type="button"
                onClick={handleEdit}
                className="text-sm font-medium text-[#2563eb] hover:text-blue-700 dark:text-blue-400"
              >
                Edit
              </button>
            )}
            ...
            <div className="flex-1 space-y-4 w-full">
                ...
                {editing ? (
                  <div className="flex flex-wrap gap-2">
                    <input
                      ...
                      className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-lg px-3 py-2 flex-1 min-w-[12rem] focus:ring-2 focus:ring-[#2563eb] focus:border-transparent outline-none transition-shadow"
                    />
                    <button
                      type="button"
                      onClick={handleSave}
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <p className="text-base font-medium text-slate-900 dark:text-slate-100">{profile.fullName || '—'}</p>
                )}
              </div>
              ...
              <div>
                ...
                {profileLoading ? (
                  ...
                  <p className="text-base text-slate-900 dark:text-slate-100">{profile.email || '—'}</p>
                )}
              </div>
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Borrow Statistics</h2>
          {historyLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="animate-pulse bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5"
                >
                  <div className="h-10 w-10 rounded-lg bg-slate-200 dark:bg-slate-700 mb-3" />
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <StatTile
                  ...
                  pillText="text-[#2563eb] dark:text-blue-300"
                />
                <StatTile
                  label="Currently Active"
                  value={currentlyActive}
                  pillBg="bg-amber-50 dark:bg-amber-950/40"
                  pillText="text-[#d97706] dark:text-amber-300"
                />
                <StatTile
                  label="Total Returned"
                  value={totalReturned}
                  pillBg="bg-green-50 dark:bg-green-950/40"
                  pillText="text-[#16a34a] dark:text-green-300"
                />
              </div>
              {historyFailed && (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Your last 3 borrow records</p>
          </div>
          {historyLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {recent.map((r) => (
                    <tr key={r.id}>
                      <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{r.bookTitle}</td>
